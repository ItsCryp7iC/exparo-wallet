...
class DetermineActionTest {
    ...
    val exparoBot = GitHubUser(GitHubUsername(Constants.EXPARO_BOT_USERNAME))
    ...
    fun `happy path -AssignIssue - multiple comments`() = testScope {
        ...
        coEvery {
            ...
            GitHubComment(exparoBot, "Random message"),
    ...
    @Test
    fun `happy path - DoNothing - exparo bot comment`() = testScope {
        // given
        coEvery {
            gitHubService.fetchIssueComments(issueNumber)
        } returns listOf(
            GitHubComment(user1, "I'm on it"),
            GitHubComment(user2, "I'm on it"),
            GitHubComment(exparoBot, "Okay, I'm on it"),
        ).right()
        coEvery {
            gitHubService.fetchIssue(issueNumber)
        } returns newIssue().right()
