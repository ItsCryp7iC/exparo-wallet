package com.exparo.data.backup

import android.content.Context
import android.net.Uri
import androidx.core.net.toUri
import com.exparo.base.legacy.SharedPrefs
import com.exparo.base.legacy.unzip
import com.exparo.base.legacy.zip
...
import com.exparo.data.repository.mapper.AccountMapper
...
class BackupDataUseCase @Inject constructor(
    ...
    suspend fun generateJsonBackup(): String {
        return withContext(dispatchersProvider.io) {
            ...
            val completeData = ExparoWalletCompleteData(
    ...
    suspend fun importJson(
        ...
        val exparoWalletCompleteData = modifiedJsonString?.let {
            json.decodeFromString<ExparoWalletCompleteData>(it)
        } ?: error("Failed to parse backup JSON.")

        onProgress(0.4)
        insertDataToDb(completeData = exparoWalletCompleteData, onProgress = onProgress)
        onProgress(1.0)
        ...
        return ImportResult(
            rowsFound = exparoWalletCompleteData.transactions.size,
            transactionsImported = exparoWalletCompleteData.transactions.size,
            accountsImported = exparoWalletCompleteData.accounts.size,
            categoriesImported = exparoWalletCompleteData.categories.size,
            failedRows = persistentListOf()
        )
    }
    ...
    private suspend fun accommodateExistingAccountsAndCategories(jsonString: String?): String? {
        ...
        val exparoWalletCompleteData = json.decodeFromString<ExparoWalletCompleteData>(jsonString)
        val replacementPairs = getReplacementPairs(exparoWalletCompleteData)
