...
import com.exparo.ExparoNavGraph
...
import com.exparo.legacy.appDesign
import com.exparo.legacy.utils.activityForResultLauncher
import com.exparo.legacy.utils.sendToCrashlytics
import com.exparo.legacy.utils.simpleActivityForResultLauncher
...
import com.exparo.wallet.ui.applocked.AppLockedScreen
import com.exparo.widget.balance.WalletBalanceWidgetReceiver
import com.exparo.widget.transaction.AddTransactionWidget
...
class RootActivity : AppCompatActivity(), RootScreen {
    @Inject
    lateinit var exparoContext: ExparoWalletCtx
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            ...
            when (appLocked) {
                ...
                true -> {
                    ExparoUI(
                        design = appDesign(exparoContext),
            ...
            ExparoMaterial3Theme(
                dark = isDarkThemeEnabled(
                    exparoDesign = appDesign(exparoContext),
                    systemDarkTheme = isSystemInDarkTheme
                ),
                isTrueBlack = appDesign(exparoContext).context().theme == Theme.AMOLED_DARK
            ) {
    ...
    private fun setupDatePicker() {
        exparoContext.onShowDatePicker = { minDate,
                                        maxDate,
                                        initialDate,
                                        onDatePicked ->
            val datePicker =
    ...
    private fun setupTimePicker() {
        exparoContext.onShowTimePicker = { initialTime,
                                        onTimePicked ->
    ...
    private fun isDarkThemeEnabled(exparoDesign: ExparoDesign, systemDarkTheme: Boolean): Boolean {
        return when (exparoDesign.context().theme) {
            Theme.LIGHT -> false
            Theme.DARK -> true
    ...
    private fun createFileLauncher() {
        ...
        exparoContext.createNewFile = { fileName, onFileCreatedCallback ->
    ...
    private fun openFileLauncher() {
        ...
        exparoContext.openFile = { onFileOpenedCallback ->
    ...
    override fun shareExparoWallet() {
        val share = Intent.createChooser(
            Intent().apply {
                action = Intent.ACTION_SEND
                putExtra(Intent.EXTRA_TEXT, Constants.URL_EXPARO_WALLET_GOOGLE_PLAY)
                type = "text/plain"
            },
            null
        )
    ...
    override fun reviewExparoWallet(dismissReviewCard: Boolean) {
