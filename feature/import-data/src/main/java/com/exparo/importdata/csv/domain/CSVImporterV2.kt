package com.exparo.importdata.csv.domain
...
import com.exparo.data.backup.CSVRow
import com.exparo.data.backup.ImportResult
...
import com.exparo.importdata.csv.ImportantFields
import com.exparo.importdata.csv.OptionalFields
import com.exparo.importdata.csv.TransferFields
...
import com.exparo.wallet.domain.pure.util.nextOrderNum
...
import com.exparo.importdata.csv.CSVRow as CSVRowNew
...
class CSVImporterV2 @Inject constructor(
    ...
    private suspend fun mapAccount(
        ...
        val colorArgb = color ?: when {
            ...
            else -> EXPARO_COLOR_PICKER_COLORS_FREE.getOrElse(newAccountColorIndex++) {
                newAccountColorIndex = 0
                EXPARO_COLOR_PICKER_COLORS_FREE.first()
            }
        }.toArgb()
    ...
    private fun mapCurrency(
        ...
        return try {
            if (currencyCode != null && currencyCode.isNotBlank()) {
                ExparoCurrency.fromCode(currencyCode)?.code ?: baseCurrency
            } else {
                baseCurrency
            }
        } catch (e: Exception) {
    ...
    private suspend fun mapCategory(
        ...
        // create new category
        val colorArgb = color ?: EXPARO_COLOR_PICKER_COLORS_FREE.getOrElse(newCategoryColorIndex++) {
            newCategoryColorIndex = 0
            EXPARO_COLOR_PICKER_COLORS_FREE.first()
        }.toArgb()
