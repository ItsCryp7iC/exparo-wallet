package com.exparo.importdata.csvimport
...
import com.exparo.legacy.domain.deprecated.logic.csv.CSVImporter
...
import com.exparo.base.legacy.getFileName
...
import com.exparo.wallet.domain.deprecated.logic.csv.CSVMapper
import com.exparo.wallet.domain.deprecated.logic.csv.CSVNormalizer
...
class ImportViewModel @Inject constructor(
    ...
    fun uploadFile(context: Context) {
        ...
        exparoContext.openFile { fileUri ->
    ...
    private suspend fun restoreCSVFile(fileUri: Uri, importType: ImportType): ImportResult {
        return com.exparo.legacy.utils.ioThread {
            val rawCSV = fileReader.read(
                uri = fileUri,
                charset = when (importType) {
                    ImportType.EXPARO -> Charsets.UTF_16
                    else -> Charsets.UTF_8
                }
            ).getOrNull()
    ...
    private suspend fun hasCSVExtension(
        context: Context,
        fileUri: Uri
    ): Boolean = com.exparo.legacy.utils.ioThread {
