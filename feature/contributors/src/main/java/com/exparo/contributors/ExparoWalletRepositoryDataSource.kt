...
class ExparoWalletRepositoryDataSource @Inject constructor(
    ...
    @Keep
    @Serializable
    data class ExparoWalletRepositoryInfo(
        @SerialName("forks")
        val forks: Int,
    ...
    private suspend fun getContributorsFromRequest(currentPage: Int): Either<String, List<ContributorDto>> =
            ...
            val contributorsDto = httpClient
                .get("https://api.github.com/repos/ItsCryp7iC/exparo-wallet/contributors") {
    ...
    suspend fun fetchRepositoryInfo(): ExparoWalletRepositoryInfo? {
        return try {
            withContext(Dispatchers.IO) {
                httpClient
                    .get("https://api.github.com/repos/ItsCryp7iC/exparo-wallet")
                    .body<ExparoWalletRepositoryInfo>()
            }
        } catch (e: Exception) {
            null
        }
